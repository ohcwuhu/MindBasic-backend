...
def list_articles(
    ...
    rows, total = get_public_articles(db, categoryId, keyword, page, pageSize)
    favorite_ids = get_favorite_ids(db, user.id, [a.id for a in rows]) if user else set()
...
def article_detail(
    ...
    article = get_public_article_or_404(db, article_id)
    favorite_ids = get_favorite_ids(db, user.id, [article.id]) if user else set()
    increment_view_count(db, article)
    return ok(
...
def favorite_article(
    ...
    is_favorite = toggle_favorite(db, user.id, article_id)
    return ok({"isFavorite": is_favorite}, trace_id=request.state.trace_id)


@categories_router.get("")
def article_categories(request: Request, db: Session = Depends(get_db)) -> dict:
    items = [
